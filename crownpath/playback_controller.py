...
def _demo_mode():
    return os.getenv("CROWNPATH_DEMO_MODE", "true").lower() == "true"


def _zone_devices(db, zone_id):
    mappings=AudioZoneDeviceRepository(db).for_zone(zone_id)
    devices=AudioDeviceRepository(db)
    results=[]
    for mapping in mappings:
        ...
        if d:
            results.append({
                ...
                "provider":d.provider,
                "configured":bool(d.external_reference),
                "primary_device":mapping.primary_device,
            })
    return results


def _effective_zone_volume(db, zone, devices):
    limits=[d["max_volume"] for d in devices if d["online"]]
    if not limits:
        return {
            "zone_id":zone.zone_id,
            "requested_volume":zone.volume,
            "effective_volume":0,
            "muted":True,
            "reason":"No online playback device",
        }
    hard_limit=min(limits)
    effective=min(zone.volume,hard_limit)
    return {
        "zone_id":zone.zone_id,
        "requested_volume":zone.volume,
        "effective_volume":0 if zone.muted else effective,
        "muted":zone.muted,
        "device_limit":hard_limit,
    }
...
def seed_devices():
    with transaction() as db:
        ...
        for item in DEFAULT_DEVICES:
            if not devices.by_id(item['device_id']):
                devices.add(AudioDevice(**item))
        for zone_id,device_id in DEFAULT_MAPPINGS:
            if not mappings.mapping(zone_id,device_id):
                mappings.add(AudioZoneDevice(mapping_id=f"CP-MAP-{uuid.uuid4().hex[:10].upper()}",zone_id=zone_id,device_id=device_id,primary_device=True))
...
def zone_devices(zone_id):
    with transaction() as db:
        return _zone_devices(db, zone_id)


def map_device(zone_id,device_id):
    with transaction() as db:
        mappings=AudioZoneDeviceRepository(db)
        if not AudioZoneRepository(db).by_id(zone_id) or not AudioDeviceRepository(db).by_id(device_id):
            return False
        if not mappings.mapping(zone_id,device_id):
            mappings.add(AudioZoneDevice(mapping_id=f"CP-MAP-{uuid.uuid4().hex[:10].upper()}",zone_id=zone_id,device_id=device_id,primary_device=True))
        return True
...
def effective_zone_volume(zone_id):
    with transaction() as db:
        ...
        if not zone:
            return None
        devices=_zone_devices(db, zone_id)
        return _effective_zone_volume(db, zone, devices)
...
def playback_state(zone_id):
    with transaction() as db:
        ...
        devices=_zone_devices(db, zone_id)
        volume=_effective_zone_volume(db, zone, devices)
